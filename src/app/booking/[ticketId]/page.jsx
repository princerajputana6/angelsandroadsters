...
function WhatsAppShareButton({ ticketId, eventTitle }) {
  const bookingUrl = `${typeof window !== 'undefined' ? window.location.origin : ''}/booking/${ticketId}`;
  const text = `🎉 I just registered for *${eventTitle}*! Here's my booking ticket:\n${bookingUrl}\n\nTicket ID: *${ticketId}*\n\nJoin me at Angels & Roadsters 🏍️🔥`;
  ...
  return (
    <a
      ...
      className="inline-flex items-center gap-2 bg-green-600 hover:bg-green-500 text-white font-semibold text-sm px-5 py-2.5 rounded-xl transition-colors"
    >
      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
        ...
        <path d="M12 0C5.373 0 0 5.373 0 12c0 2.127.557 4.126 1.528 5.855L.057 23.744l6.043-1.585A11.944 11.944 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.808 9.808 0 01-5.006-1.368l-.358-.214-3.724.977.995-3.63-.234-.374A9.818 9.818 0 012.182 12C2.182 6.57 6.57 2.182 12 2.182S21.818 6.57 21.818 12 17.43 21.818 12 21.818z" />
      </svg>
      Share on WhatsApp
    </a>
  );
      ...
      <div className="container-x max-w-4xl mx-auto">
        ...
        {isProfileComplete() && booking.qrCode && (
          <div className="card p-6 mb-6 text-center bg-green-500/10 border-2 border-green-500/30">
            ...
            <p className="text-xs text-charcoal-500 mb-4">Keep this QR code safe and accessible on your phone</p>
            {/* Share button */}
            <div className="flex justify-center">
              <WhatsAppShareButton
                ticketId={booking.ticketId}
                eventTitle={booking.event?.title || 'Angels & Roadsters Event'}
              />
            </div>
          </div>
        )}
